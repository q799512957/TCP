...
//			LogUtil.getLogger().error("ascii2String", e);
            LoggerFactory.getLogger(Converter.class).error(e.getMessage());
        }
        return aString.trim();
    }
    ...
    public static float bcd2Float(byte[] b, int startIndex, int length, int dotlen) {
        byte[] s = new byte[length];
        System.arraycopy(b, startIndex, s, 0, length);
     ...
     */
    public static String bcd2DotString(byte[] bytes, int startIndex, int len, int dot) {
        byte[] b = new byte[len];
        System.arraycopy(bytes, startIndex, b, 0, len);
        String temp = bcd2Str(b);
        temp = temp.substring(0, temp.length() - dot) + "." + temp.substring(temp.length() - dot);
    ...
    public static byte hexStr2Byte(String hex) {
        ...
        int n = hex.length() / 2;
        for (int i = 0; i < n; i++) {
            b = (byte) Integer.parseInt(hex.substring(2 * i, 2 * i + 2), 16);
        }
        return b;
    }
     ...
     * wujinzhong 0610
     *
     * @param bts Description of Parameter
